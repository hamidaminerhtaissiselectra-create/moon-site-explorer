          ...
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-card rounded-xl p-8 card-shadow border border-border">
              ...
              <div className="flex gap-2">
                {typeOptions.map((t) => (
                  <button
                    ...
                    className={`flex-1 px-3 py-2 rounded-lg border-2 text-sm font-bold transition-all ${requestType === t.id ? t.color + " border-current" : "border-border/80 bg-background text-foreground/80"}`}
                  >
                    {t.label}
                  </button>
                ))}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <Input required placeholder="Votre nom" className="bg-background border-border/80 text-foreground font-medium" />
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-foreground mb-1.5">Email</label>
                  <Input required type="email" placeholder="votre@email.com" className="bg-background border-border/80 text-foreground font-medium" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-foreground mb-1.5">Téléphone</label>
                  <Input required type="tel" placeholder="06 XX XX XX XX" className="bg-background border-border/80 text-foreground font-medium" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-bold text-foreground mb-1.5">Message</label>
                <Textarea required placeholder="Décrivez votre besoin..." rows={4} className="bg-background border-border/80 text-foreground font-medium" />
              </div>
